...
dat <- epcall |> 
  ...
  mutate(
    ...
    type1 = factor(type1, levels = c('Fresh', 'Marine')),
    type2 = case_when(
...
This document is a quick summary of the distribution of nitrate data collected by the Environmental Protection Commission of Hillsborough County.  Source data are available [here](https://epcbocc.sharepoint.com/:f:/s/Share/EiypSSYdsEFCi84Sv_6-t7kBUYaXiIqN0B1n2w57Z_V3kQ?e=NdZQcU){target="_blank"}. Nitrates and nitrites have been collectively measured since the early 1980s, with only nitrate quantified prior.
...
toplo <- dat |> 
  ...
  mutate(
    param = factor(param, levels = c('Nitrates_mgL', 'Nitrates_Nitrites'), 
                   labels = c('Nitrate only', 'Nitrates/Nitrites')),
...
```{r}
#| out-width: 100%
pal <- colorFactor(c("skyblue", "navy"), domain = c("Fresh", "Marine"), levels = c('Fresh', 'Marine'))
...
leaflet(tomap) |> 
  ...
  addLegend(
    ...
    colors = c("skyblue", "navy"), 
    labels = c("Fresh", "Marine"), 
    opacity = 0.8,
    title = NULL
  )
```
...
toplo <- dat |> 
  ...
  na.omit(value) |> 
  mutate(
    type1 = factor(type1, levels = rev(levels(type1))),
...
```{r}
#| out-width: 100%
pal <- colorFactor(palette = c('navy', 'skyblue', 'green', 'purple'), domain = c("Fish consumption marine", "Fish consumption fresh", "Shellfish", "Potable water supply"), levels = c('Fish consumption marine', 'Fish consumption fresh', 'Shellfish', 'Potable water supply'))
# create leaflet map of stations with color by type
tomap <- dat |> 
...
leaflet(tomap) |> 
  ...
  addLegend(
    position = "bottomright", 
    colors = c('navy', 'skyblue', '#33ff33', 'purple'), 
...
toplo <- dat |> 
  ...
  na.omit(value) |> 
  mutate(
    type2 = factor(type2, levels = rev(levels(type2))),
